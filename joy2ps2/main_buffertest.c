...
unsigned char stack[MAXSTACKSIZE];
int stackptr = 0;
...
void sendPS2fromstack(unsigned char code)
{
...
void sendCodeMR(unsigned char key, uint16_t release)
{
	...
	if (extn)
	{
		if (stackptr < MAXSTACKSIZE - 2) // Ignoramos scancodes si supera el tamaño del buffer
		{
			sendPS2(0xE0);
		}
		else
	...
	if (key && release)
	{
		if (stackptr < MAXSTACKSIZE - 1) // Ignoramos scancodes si supera el tamaño del buffer
		{
			sendPS2(0xF0);
		}
		else
	...
	if (key)
	{
		if (stackptr < MAXSTACKSIZE) // Ignoramos scancodes si supera el tamaño del buffer
		{
			sendPS2(key);
		}
	}
...
int main()
{
	...
	while (1) {
		...
		if (stackptr > 0) // Liberamos buffer de scancodes
		{
			sendcode = pop;
			while ((sendcode == 0xE0 || sendcode == 0xF0) && stackptr > 0)
			{
				sendPS2fromstack(sendcode);
				sendcode = pop;
			}			
			sendPS2fromstack(sendcode);
		}
